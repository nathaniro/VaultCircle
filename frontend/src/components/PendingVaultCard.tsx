      ...
      <Link href={`/vault/${revealedVault.vaultId}`} className="surface-card surface-card-interactive vault-morph-card vault-morph-ready group block">
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-3 text-2xl font-semibold text-ink-50">#{revealedVault.vaultId}</p>
          </div>
          <div>
            <p className="stat-label">Total vault value</p>
            <p className="mt-3 text-2xl font-semibold text-amber-700 dark:text-amber-300">
              {satsTosBTC(revealedVault.totalVaultValue || revealedVault.liquidBalance)} sBTC
            </p>
          </div>
          <div>
            <p className="stat-label">Members at launch</p>
            <p className="mt-3 text-sm font-semibold text-ink-50">{revealedVault.memberCount} signers active</p>
          </div>
          <div>
            <p className="stat-label">Approval threshold</p>
            <p className="mt-3 text-sm font-semibold text-ink-50">{revealedVault.thresholdPercent}% before assets move</p>
          </div>
        </div>

        <div className="mt-6 flex items-center justify-between border-t border-overlay/10 pt-6 text-sm text-emerald-800/80 dark:text-emerald-50/80">
          <span>Open the new vault and continue setup</span>
          <span className="transition duration-200 group-hover:translate-x-1 group-hover:text-emerald-600 dark:group-hover:text-emerald-100">-&gt;</span>
        </div>
      </Link>
    );
  }
  ...
  return (
    <div className="surface-card vault-morph-card border-orange-400/20 bg-orange-400/[0.06]">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div>
          <p className="stat-label">Members at launch</p>
          <p className="mt-3 text-2xl font-semibold text-ink-50">{pendingVault.memberCount}</p>
        </div>
        <div>
          <p className="stat-label">Approval threshold</p>
          <p className="mt-3 text-2xl font-semibold text-amber-700 dark:text-amber-300">{pendingVault.thresholdPercent}%</p>
        </div>
        <div>
          <p className="stat-label">Yield configuration</p>
          <p className={`mt-3 text-sm font-semibold ${pendingVault.yieldEnabled ? "text-emerald-700 dark:text-emerald-300" : "text-ink-300"}`}>
            {pendingVault.yieldEnabled ? "Yield path enabled" : "Yield path disabled"}
          </p>
        </div>
        <div>
          <p className="stat-label">Submitted transaction</p>
          <p className="mt-3 break-all font-mono text-xs text-ink-300">{pendingVault.txId}</p>
        </div>
        {typeof pendingVault.vaultId === "number" && (
          <div>
            <p className="stat-label">Confirmed vault id</p>
            <p className="mt-3 text-sm font-semibold text-ink-50">#{pendingVault.vaultId}</p>
          </div>
        )}
      </div>

      <div className="mt-6 flex flex-col gap-3 border-t border-overlay/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
